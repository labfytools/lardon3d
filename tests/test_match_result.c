...
static bool run_test(void) {
  ...
  CHECK(lardon3d_project_db_open(database_path, &database, error) ==
        LARDON3D_PROJECT_DB_OK);
  CHECK(database && lardon3d_project_db_schema_version(database) == LARDON3D_PROJECT_DB_SCHEMA_VERSION);
  ...
  database = NULL;
  CHECK(query_integer(
      v9_path, "SELECT value FROM metadata WHERE key='schema_version'",
  ...
  CHECK(lardon3d_project_db_open(failed_v10_path, &database, error) ==
            LARDON3D_PROJECT_DB_OK &&
        lardon3d_project_db_schema_version(database) == LARDON3D_PROJECT_DB_SCHEMA_VERSION);
  lardon3d_project_db_close(database);
  database = NULL;
